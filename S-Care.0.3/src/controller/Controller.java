package controller;
import interfaces.group.InterfaceQuestionEvaluationBis;
...
import helpers.FillingDB;
import questions.QuestionEvaluation;
import questionnaires.QuestionnaireBASDAI;
import questionnaires.QuestionnaireBASFI;
import utils.Constants;
import utils.Constants.QuestionnaireType;
...
//	}
	...
	/***************** SPA ********************************/
	public static ArrayList<QuestionEvaluation> questionEvalBASDAI;
	public static ArrayList<QuestionEvaluation> questionEvalBASFI;

	public static ArrayList <QuestionEvaluation> questionEvaluation;

	public static ArrayList<ArrayList<String>> listQuestionEvaluationBASDAI;
	public static ArrayList<ArrayList<String>> listQuestionEvaluationBASFI;

	public static QuestionEvaluation questionEvaluationBASDAI;
	public static QuestionEvaluation questionEvaluationBASFI;
	
	static QuestionnaireBASDAI questionnaireBASDAI;
	static QuestionnaireBASFI questionnaireBASFI;
	
	
	public static void fillDB(Context mContext){
		FillingDB.Fill(mContext);
	}	
	
	//Initialize questionnaire
	public static void StartQuestionnaire (Context mContext)
	{
		if (MemoryManager.currentQuestionnaire == QuestionnaireType.BASDAI_TYPE)
		{
	        fillQuestionEvaluationListForBASDAI(mContext);
		...
		{
			fillQuestionEvaluationListForBASFI(mContext);
			InterfaceQuestionEvaluationBis.setListQuestion();
			InterfaceQuestionEvaluationBis.i = -1;			
		}
	}
	...
	public static void fillQuestionEvaluationListForBASDAI(Context mContext){
		questionEvaluation=new ArrayList <QuestionEvaluation>();

		listQuestionEvaluationBASDAI=new ArrayList<ArrayList<String>>();

		DBAdapter dba = new DBAdapter(mContext);
		dba.open();
		listQuestionEvaluationBASDAI=dba.GetListQuestion_BASDAI();
		for (int i=0; i<listQuestionEvaluationBASDAI.size();i++)
		{
			ArrayList<String> question=new ArrayList<String>();	

			question=listQuestionEvaluationBASDAI.get(i);


				QuestionEvaluation Q = new QuestionEvaluation (Integer.parseInt(question.get(0)), question.get(1), Integer.parseInt(question.get(2)), question.get(3).toString());
				Q.setReponse(0);
				questionEvaluation.add(Q);
	...
	public static void fillQuestionEvaluationListForBASFI(Context mContext){
		questionEvaluation=new ArrayList <QuestionEvaluation>();

		listQuestionEvaluationBASFI=new ArrayList<ArrayList<String>>();

		DBAdapter dba = new DBAdapter(mContext);
		dba.open();
		listQuestionEvaluationBASFI=dba.GetListQuestion_BASFI();
		for (int i=0; i<listQuestionEvaluationBASFI.size();i++)
		{
			ArrayList<String> question=new ArrayList<String>();	

			question=listQuestionEvaluationBASFI.get(i);
	...
	public static void ActionCalculScoreBASDAI(Context mContext1)
	{
		questionnaireBASDAI=new QuestionnaireBASDAI(1,"BASDAI", questionEvaluation);		
		float score=questionnaireBASDAI.CalculScore();	
		questionnaireBASDAI.setScore(score);
		Toast.makeText(mContext1, "Le score pour le BASDAI est : " + score ,Toast.LENGTH_LONG).show();
		
		dba = new DBAdapter(mContext1);
		dba.open();
		SharedPreferences IdUser = mContext1.getSharedPreferences("IdUser", MODE_PRIVATE);
		int id = IdUser.getInt("IDPatient", 0);
		SimpleDateFormat sdf = new SimpleDateFormat("ddMMyyyy HHmmss");
		String currentDateandTime = sdf.format(new Date());
		
		
		dba.InsertScore(questionnaireBASDAI.getScore(), currentDateandTime, "BASDAI", id);
	...
	public static void ActionCalculScoreBASFI(Context mContext1)
	{
		
		questionnaireBASFI=new QuestionnaireBASFI(1,"BASFI", questionEvaluation);		
		float score=questionnaireBASFI.CalculScore();	
		questionnaireBASFI.setScore(score);
		Toast.makeText(mContext1, "Le score pour le BASFI est : " + score ,Toast.LENGTH_LONG).show();
		...
		dba.InsertScore(questionnaireBASFI.getScore(), currentDateandTime, "BASFI", id);
